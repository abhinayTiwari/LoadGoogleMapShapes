...
function initialize() {
  ...
  google.maps.event.addListener(drawingManager, "overlaycomplete", function (
    ...
    switch (event.type) {
      ...
      case google.maps.drawing.OverlayType.CIRCLE:
        map.data.add(
          new google.maps.Data.Feature({
            properties: { radius: event.overlay.getRadius() },
  ...
  //function to generate Geo JSON Data
      ...
      fx = function (g, t) {
        ...
        switch (t) {
          ...
          default:
            ...
            if (t == "LinearRing" && that[0] !== that[that.length - 1]) {
...
function downloadJsonData(event) {
  if (map.getGeoJson().features.length == 0) {
    alert(
      "Please draw a shape first in the given map using the 'Draw a Shape' tool"
    );
    return;
  }
...
document
  ...
  .addEventListener("click", downloadJsonData);
